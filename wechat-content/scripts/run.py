...
def copy_eligibility(payload: dict) -> dict:
    ...
    def effective_status(item):
        raw_status=item.get("verification_status")
        if not raw_status and payload.get("status")=="ready_for_human_review":
            return "verified"
