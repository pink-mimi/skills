...
class WechatContentTests(unittest.TestCase):
    ...
    def test_ai_discovery_renders_reviewable_wechat_package(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertNotIn("images/AI发现-01.png", article)
            self.assertNotIn("使用场景示意图", article)
            ...
            self.assertFalse((out / "images/AI发现-01.png").exists())
    ...
    def test_github_schema_v2_uses_lightweight_project_card(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("<!-- github-tags:AI Agent 工具 -->", article)
            self.assertIn("一句话推荐", article)
            self.assertIn("它是什么", article)
            self.assertIn("为什么值得看", article)
