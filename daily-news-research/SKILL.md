...
3. 默认只选与中国境内直接相关的“昨日国内大事”；国际等内容必须证明对国内读者有直接影响。
4. 打开原始链接核验并补齐发生了什么、为什么重要、普通人需要注意什么、克制提醒和关键词。RSS 摘要不能直接作为发布级内容。
5. 运行 `build` 完成时间过滤、去重、分类与筛选；缺少深度字段时保持 `needs_review`。
6. 运行 `verify`；只把内容包交给平台制作 Skill，不在本 Skill 中写公众号文章。
