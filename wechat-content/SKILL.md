...
3. 新闻读者正文必须包含 `what_happened`、`why_it_matters`、`reader_action` 和 `keywords`；缺少时不得假装成稿完整，必须标记 `needs_review` 并禁用复制按钮。读者字段完整时允许复制；存在 `partial` 或 `unverified` 时，必须在复制区外提示发布前核验，并明确不得把成功复制视为发布就绪。`editor_note`、核验状态和审核要求只供运营者查看，不得进入读者正文。
