...
读取标准内容包，自动选择对应栏目结构，生成标题、摘要、可一键复制的微信 HTML、内容配图，以及“左长、右方”的组合封面。新闻、GitHub 热门与 AI 新发现使用独立内容模板，不再共用占位图。
...
| `ai-discovery` | AI 坐标、用途、适合谁、费用限制、风险、官方地址 | AI 产品和工作流节点 |
...
- 正文完整且所有新闻至少达到部分核验时允许复制，并在复制区外提示发布前复核；真正未核验或字段缺失时禁用复制。
- 关键词使用一条连续的关键词信息条，以 `｜` 分隔并自然换行，不使用零碎胶囊标签。
- `daily-news`、`github-hot` 与 `ai-discovery` 使用各自独立的读者规则；AI 新发现没有实际测试时，不写亲身体验。
