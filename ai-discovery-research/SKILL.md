...
围绕“AI 新发现”栏目发现近期值得普通读者了解的 AI 模型、产品、应用和实践案例。搜集不限制国内或国外，但最终优先选择最近 14 天内有明确热度信号、普通读者能理解或尝试的对象。每期候选池建议 8-12 个，聚焦复核至少 3 个，最终只入选 1 个重点对象。
...
2. 运行 `collect`，或用 `--input` 载入离线候选。候选来源可以包括官方博客、发布页、文档、论文页、Hugging Face、ModelScope、GitHub、主流技术媒体和社区讨论。
   - 国内发现入口优先覆盖：ModelScope 魔搭、AI 工具集、机器之心、量子位、少数派、36 氪、InfoQ 中文、知乎、即刻、B 站，以及智谱、Kimi、通义、豆包、腾讯混元、MiniMax、阶跃星辰、讯飞星火、百度文心等官方发布渠道。
   - 第三方平台只用于发现线索和热度信号，功能、价格、开放地区、隐私和授权等确定性事实必须回到官方来源核验。
3. 确定性事实必须回到官方来源核验：产品主页、官方文档、价格页、隐私政策、服务条款、区域可用性说明、论文页、GitHub README、Hugging Face model card 或发布公告。
4. 对每条候选补齐名称、类型、公司/团队、链接、发布时间或发现时间、官方来源、用途、适合谁、不适合谁、使用场景、费用/限制、大陆可用性、使用门槛、热度信号、风险、核验状态、核验等级和推荐理由。
   - `verified` 或 `partial` 条目的官方来源都必须记录 `verified_at`。
   - `verification_grade: "C"` 的候选不得入选。
   - 入选对象至少需要 2 个 `popularity_signals`，例如官方近期发布、两家以上媒体报道、ModelScope/Hugging Face/GitHub 互动增长、知乎/即刻/B 站等社区讨论、普通用户可直接试用路径。
