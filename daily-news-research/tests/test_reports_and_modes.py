...
class ReportsAndModesTests(unittest.TestCase):
    ...
    def test_all_writes_auditable_tiered_research_outputs(self):
        with tempfile.TemporaryDirectory() as temp:
            result=self.run_cli("all","--input",str(FIXTURE),"--output-root",temp,"--run-at","2026-07-19T06:20:00+08:00")
            self.assertEqual(result.returncode,0,result.stdout+result.stderr)
            out=Path(temp)/"daily-news/2026-07-19"
            ...
            history=Path(temp)/"daily-news/source-health-history.json"
    ...
    def test_rebuild_is_offline_and_does_not_claim_fresh_verification(self):
        with tempfile.TemporaryDirectory() as temp:
            first=self.run_cli("all","--input",str(FIXTURE),"--output-root",temp,"--run-at","2026-07-19T06:20:00+08:00")
            self.assertEqual(first.returncode,0,first.stdout+first.stderr)
            rebuilt=self.run_cli("all","--mode","rebuild","--output-root",temp,"--run-at","2026-07-19T06:20:00+08:00")
            self.assertEqual(rebuilt.returncode,0,rebuilt.stdout+rebuilt.stderr)
            package=json.loads((Path(temp)/"daily-news/2026-07-19/content-package.json").read_text(encoding="utf-8"))
