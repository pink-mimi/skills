...
3. 新闻条目必须包含 `what_happened`、`why_it_matters`、`reader_action`、`editor_note` 和 `keywords`；缺少时不得假装成稿完整，必须标记 `needs_review`、禁用复制按钮，并把审核提示放在复制区域之外。正文完整且全部条目至少达到部分核验 `partial` 时允许复制，但必须在复制区外提示发布前复核；真正 `unverified` 时仍禁止复制。读者正文不得出现内部占位语。
...
7. 运行 `verify`。`OK` 表示完整核验后可复制，`OK_WITH_PARTIAL_REVIEW` 表示允许复制但存在部分核验内容、必须发布前复核；`STRUCTURE_OK_CONTENT_NEEDS_REVIEW` 只表示文件结构正常，不得称为验证通过。再用微信手机预览人工检查。
